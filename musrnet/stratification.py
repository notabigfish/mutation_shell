...
def _compute_freesasa_annotation(structure, chain_id: str, label_chain_ids: list[str]) -> dict[tuple[int, str], dict[str, float]]:
    calc_result = freesasa.calcBioPDB(structure, options={"hetatm": False, "skip-unknown": True})

    if len(calc_result) == 3:
        result, _sasa_classes, residue_areas = calc_result
    else:
        result, _sasa_classes = calc_result
        residue_areas = result.residueAreas()

    chain_areas = {}
    actual_chain_id = None

    for candidate_chain_id in dict.fromkeys([chain_id] + label_chain_ids):
        candidate_areas = residue_areas.get(candidate_chain_id, {})
        if candidate_areas:
            chain_areas = candidate_areas
            actual_chain_id = candidate_chain_id
            break

    if not chain_areas: return {}

    mapping: dict[tuple[int, str], dict[str, float]] = {}

    for residue_label, area in chain_areas.items():
        residue_label = str(residue_label).strip()

        m = re.match(r"^(-?\d+)(.*)$", residue_label)
        if not m: continue
        ...
        icode = m.group(2).strip()

        resname = str(getattr(area, "residueType", "")).upper().strip()
        aa1 = AA3_TO_1.get(resname, resname if len(resname) == 1 else None)

        max_asa = MAX_ASA.get(aa1, np.nan) if aa1 else np.nan

        if max_asa and np.isfinite(max_asa):
            rsa = float(area.total) / float(max_asa)
            rsa = float(np.clip(rsa, 0.0, 1.0))
        else:
            rsa = float("nan")

        mapping[(resseq, icode)] = {"rsa": rsa, "freesasa_chain_id": actual_chain_id, "residue_type": resname}
...
DSSP_SAFE_CHAIN_IDS = list(string.ascii_uppercase + string.ascii_lowercase + string.digits)

def _first_mmcif_value(mmcif: dict, keys: list[str], default):
    for key in keys:
        value = mmcif.get(key)
        if value is None:
            continue
...
def _cryst1_from_mmcif(cif_path: Path) -> str:
    mmcif = MMCIF2Dict(str(cif_path))

    a = float(_first_mmcif_value(mmcif, ["_cell.length_a"], 1.0))
    b = float(_first_mmcif_value(mmcif, ["_cell.length_b"], 1.0))
    c = float(_first_mmcif_value(mmcif, ["_cell.length_c"], 1.0))
    alpha = float(_first_mmcif_value(mmcif, ["_cell.angle_alpha"], 90.0))
    beta = float(_first_mmcif_value(mmcif, ["_cell.angle_beta"], 90.0))
    gamma = float(_first_mmcif_value(mmcif, ["_cell.angle_gamma"], 90.0))

    sg = _first_mmcif_value(
        mmcif,
        ["_symmetry.space_group_name_H-M", "_space_group.name_H-M_alt"],
        "P 1",
    )
    z = _first_mmcif_value(mmcif, ["_cell.Z_PDB"], "")
...
def _is_dssp_protein_residue(res) -> bool:
    resname = str(res.get_resname()).upper().strip()

    if resname in AA3_TO_1:
        return "CA" in res

    try:
        return res.id[0] == " " and is_aa(res, standard=True) and "CA" in res
    except Exception:
        return False
    
def _clean_residue_for_dssp(res, new_resseq: int):
    r = res.copy()

    # 强制变成 ATOM 标准 residue id；避免 HETATM/MSE/插入码搞 DSSP old format
    r.id = (" ", int(new_resseq), " ")

    # 简单处理 altloc：保留 blank/A/1
    for atom_id in list(r.child_dict.keys()):
        atom = r.child_dict[atom_id]

        if hasattr(atom, "disordered_select") and hasattr(atom, "child_dict"):
            selected = None
            for altloc in (" ", "A", "1"):
    ...
    # MSE -> MET，避免 Structure/DSSP mismatch
    if str(r.get_resname()).upper().strip() == "MSE":
        r.resname = "MET"

        if "SE" in r.child_dict and "SD" not in r.child_dict:
            se_atom = r.child_dict["SE"]
            r.detach_child("SE")
            try:
                se_atom.id = "SD"
                se_atom.name = "SD"
...
def _make_dssp_clean_pdb(
    structure,
    cif_path: Path,
    pdb_id: str,
    target_chain_aliases: list[str],
    out_pdb: Path,
):
    """
    Make DSSP-safe PDB:
      ...
      - no HETATM ligands/waters/ions
      - 1-char chain IDs
      - residue numbers renumbered to 1..N
      - HEADER + CRYST1 added

    Return:
      renumber_map[(safe_chain_id, safe_res_id)] = original info
    """
    src_model = next(structure.get_models())

    protein_chains = []
    for chain in src_model:
        if any(_is_dssp_protein_residue(res) for res in chain):
            protein_chains.append(chain)

    if not protein_chains:
        raise ValueError("no_protein_chain_for_dssp_clean_pdb")

    target_alias_set = set(map(str, target_chain_aliases))

    status = "mkdssp_clean_pdb_all_protein_chains"

    # PDB chain ID 只有 1 字符；链太多时只保留目标链，避免 PDB 格式本身放不下
    if len(protein_chains) > len(DSSP_SAFE_CHAIN_IDS):
        protein_chains = [ch for ch in protein_chains if str(ch.id) in target_alias_set]
        status = "mkdssp_clean_pdb_target_chain_only_too_many_chains"

    if not protein_chains:
        raise ValueError(
            f"target_chain_not_found_for_dssp_clean_pdb: aliases={sorted(target_alias_set)}"
        )

    new_structure = Structure(f"{pdb_id}_dssp_clean")
    new_model = Model(0)

    renumber_map = {}
    chain_map = {}

    for chain_index, src_chain in enumerate(protein_chains):
        safe_chain_id = DSSP_SAFE_CHAIN_IDS[chain_index]
        chain_map[safe_chain_id] = str(src_chain.id)

        new_chain = Chain(safe_chain_id)
        new_resseq = 1

        for res in src_chain:
            if not _is_dssp_protein_residue(res):
                continue

            original_resseq = int(res.id[1])
            original_icode = str(res.id[2]).strip()
            original_resname = str(res.get_resname()).upper().strip()

            clean_res = _clean_residue_for_dssp(res, new_resseq)

            if len(clean_res) == 0:
                continue

            safe_res_id = (" ", int(new_resseq), " ")
            renumber_map[(safe_chain_id, safe_res_id)] = {
                "chain_id": str(src_chain.id),
                "resseq": original_resseq,
                "icode": original_icode,
                "resname": original_resname,
            }

            new_chain.add(clean_res)
            new_resseq += 1
    ...
    if len(new_model) == 0:
        raise ValueError("no_residue_left_after_dssp_cleaning")
    ...
    cryst1 = _cryst1_from_mmcif(cif_path)

    with open(raw_pdb) as fin, open(out_pdb, "w") as fout:
        fout.write(f"HEADER    DSSP CLEAN PDB                         01-JAN-00   {pdb_id[:4].upper():>4}\n")
        fout.write(cryst1)
    ...
    return renumber_map, chain_map, status

def _run_dssp_clean_pdb(
    structure,
    cif_path: Path,
    pdb_id: str,
    target_chain_aliases: list[str],
    ...
    with tempfile.TemporaryDirectory() as td:
        clean_pdb = Path(td) / f"{pdb_id.lower()}_dssp_clean.pdb"

        renumber_map, chain_map, status = _make_dssp_clean_pdb(
            structure=structure,
            cif_path=cif_path,
            pdb_id=pdb_id,
            target_chain_aliases=target_chain_aliases,
            out_pdb=clean_pdb,
        )

        clean_parser = PDBParser(QUIET=True)
        clean_structure = clean_parser.get_structure(f"{pdb_id}_dssp_clean", str(clean_pdb))
        ...
        return dssp_result, renumber_map, chain_map, status
    ...
    try:
        ...
        try:
            with py_warnings.catch_warnings(record=True) as caught_warnings:
                ...
                dssp_result, dssp_renumber_map, dssp_chain_map, dssp_status = _run_dssp_clean_pdb(
                    structure=structure,
                    cif_path=tmp_path,
                    pdb_id=pdb_id,
                    target_chain_aliases=label_chain_ids,
                )

            warning_text = "; ".join(str(w.message) for w in caught_warnings)
            dssp_reason = dssp_status if not warning_text else f"{dssp_status}; warnings: {warning_text}"

        except Exception as exc:
            warning_text = "; ".join(
                str(w.message) for w in caught_warnings
            ) if "caught_warnings" in locals() else ""

            dssp_reason = f"mkdssp_clean_pdb_failed: {exc}; warnings: {warning_text}"
        if dssp_result is not None and dssp_renumber_map is not None:
            chain_cache = {}
            ...
            for key in dssp_result.keys():
                ...
                original = dssp_renumber_map.get((safe_chain_id, safe_res_id))
                if original is None:
                    continue

                chain = original["chain_id"]
                resseq = int(original["resseq"])
                icode = str(original["icode"]).strip()
            ...
            for alias_chain_id in label_chain_ids:
                if alias_chain_id in chain_cache:
                    _save_json(cache_path, chain_cache[alias_chain_id])

                    if cache_key in chain_cache[alias_chain_id]:
                        result = chain_cache[alias_chain_id][cache_key]
                        result = dict(result)
                        result["annotation_status"] = (
                            f'{result.get("annotation_status", "dssp")}'
                            f":chain_id_mapped:{chain_id}->{alias_chain_id}"
                        )
                        return result
        try:
...
def build_biological_annotations(
    ...
    df["wt_pos_pdb_number"] = pd.to_numeric(df["wt_pos_pdb_number"], errors="coerce")
    ...
    def fetch_external_data(row_dict: dict[str, Any]) -> tuple[float, dict[str, Any]]:
        ...
        if pd.isna(row_dict.get("wt_pos_pdb_number")):
            _warn(
                warnings,
                f"{row_dict['sample_id']} missing wt_pos_pdb_number; DSSP/RSA annotation skipped",
            )
            dssp = {
                "rsa": float("nan"),
                "secondary_structure_code": "unknown",
                "annotation_status": "missing_wt_pos_pdb_number",
            }
        ...
        dssp = load_or_compute_dssp_annotation(
            ...
            residue_number=int(row_dict["wt_pos_pdb_number"]),
...
def compute_sample_metrics(
    ...
    warnings = warnings if warnings is not None else []
    rows = []
    for sample_id, sample_df in tqdm(df.groupby("sample_id", sort=False), desc="Compute sample metrics", total=df["sample_id"].nunique()):
        errors = np.abs(
