...
def _epoch_enabled_weight(base_weight: float, current_epoch: float, warmup: float, start_epoch: float | None) -> float:
    ...
    if current_epoch >= start_epoch + warmup:
        return base_weight
    return base_weight * (current_epoch - start_epoch + 1.0) / warmup
...
def compute_losses(
    ...
    radius_loss = disp_loss.new_tensor(0.0)
    if use_radius_loss and "radius" in outputs:
        radius_loss = torch.abs(outputs["radius"] - batch.y_radius.view(-1)).mean()

    class_loss = disp_loss.new_tensor(0.0)
    if use_class_loss and "class_logit" in outputs:
        class_loss = F.cross_entropy(outputs["class_logit"], batch.y_class.view(-1))
    ...
    total = (
        disp_loss
        + w_perturbed_eff * pert_loss
        + w_background * background_penalty
        + w_radius_eff * radius_loss
