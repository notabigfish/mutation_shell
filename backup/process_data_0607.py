...
MIN_LEN = 32
N_CPUS = max(1, cpu_count() - 2) if args.num_workers < 0 else args.num_workers
...
def read_pdb(pdb_id, chain_id):
	if args.pdb_format == 'pdb':
		pdb_path = f'{args.pdb_root}/{args.pdb_version}/pdb/pdb{pdb_id.lower()}.ent.gz'
		pdb = PandasPdb().read_pdb(pdb_path)
	elif args.pdb_format == 'mmcif':
		pdb_path = f'{args.pdb_root}/{args.pdb_version}/pdb/{pdb_id.lower()}.cif.gz'
		pdb = PandasMmcif().read_mmcif(pdb_path)
...
# 				match = re.search(
# 					r"SEQADV\s+(\S+)\s+(\S+)\s+(\S)\s+(\d+)\s+(\S?)\s+(\S+)\s+(\S+)\s+(\S+)\s+(\d+)\s+(.*)",
# 					line)
# 				if match:
# 					seqadv_lines.append(list(match.groups())[:-1])
# 	return seqadv_lines
...
def parse_seqadv_line(line):
	...
	if line.startswith('SEQADV'):
		...
		if len(tokens) >= 12:
			...
			database = "" if database in ('?', '.') else database
			db_accession = "" if db_accession in ('?', '.') else db_accession
			db_seqnum = "" if db_seqnum in ('?', '.') else db_seqnum
...
def validate_mutation_record(line):
	pdb_id = line[0].lower()
	chain_id = line[2]
	if pdb_id in ['5z8w', '7why', '7a7y', '7a7w', '7ttb']:
		# continue
	...
	if len(seq) < MIN_LEN or len(seq) > MAX_LEN:
		...
		return None
	# unique_residue_numbers = df['residue_number'].unique()
	# id_mapping = {res_num: idx for idx, res_num in enumerate(unique_residue_numbers)}
...
# 	if pos not in id_mapping:
# 		log_skip("pos_not_in_id_mapping", pdb_id=pdb_id, chain_id=chain_id)
# 		continue
# 	correct_index = id_mapping[pos]
# 	expected_mut_aa = three_to_one(mut_type)
# 	observed_aa = seq[correct_index]
...
# 			pdb_id=pdb_id,
# 			chain_id=chain_id,
# 			extra=f"pdb_resnum={pos}; idx0={correct_index}; expected={expected_mut_aa}; observed={observed_aa}",
# 		)
# 		# return None
# 		continue
# 	line = list(line)
...
corrected_df_seqadv = list(progress_map(validate_mutation_record, df_seqadv.itertuples(index=False, name=None), total=len(df_seqadv), n_cpu=N_CPUS))
corrected_df_seqadv = [mutant for mutant in corrected_df_seqadv if mutant is not None]
...
def extract_chain_sequences(filepath):
	...
	try:
		...
		if args.pdb_format == 'pdb':
			...
			pdb = PandasMmcif().read_mmcif(filepath)
			has_multi_model = pdb.df['ATOM']['pdbx_PDB_model_num'].nunique() > 1
			pdb.df['ATOM'] = pdb.df['ATOM'].rename(columns=rename_dict)
			pdb.df['ATOM']['residue_number'] = pd.to_numeric(pdb.df['ATOM']['residue_number'], errors='coerce')
