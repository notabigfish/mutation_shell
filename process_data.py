...
def get_seq_and_index_to_pdb_mapping(pdb_df):
	seq, id_mapping = get_seq_and_mapping(pdb_df)
	if seq is None or id_mapping is None:
		return None, None

	idx_to_pdb = {}
	for (resnum, ins_code), idx in id_mapping.items():
		try:
			resnum = int(float(resnum))
		except Exception:
			continue
...
def get_structure_path(pdb_id, pdb_format, pdb_root, pdb_version):
	...
	if pdb_format == 'pdb': return os.path.join(pdb_root, pdb_version, 'pdb', f'pdb{pdb_id}.ent.gz')
	if pdb_format == 'mmcif': return os.path.join(pdb_root, pdb_version, 'pdb', f'{pdb_id}.cif.gz')
	raise ValueError(f"Unknown pdb_format: {pdb_format}")
...
@func_set_timeout(30)
def get_release_date(pdb_id, pdb_format, pdb_root, pdb_version):
...
def build_wt_mut_fasta_record_group(item, pdb_format, pdb_root, pdb_version):
...
def extract_chain_sequences(filepath, pdb_format, pdb_root, pdb_version):
	"""
...
def process_batch(file_batch, pdb_format, pdb_root, pdb_version):
	"""Process a batch of files and return combined results"""
	batch_results = []
	for filepath in file_batch:
		try:
			results = extract_chain_sequences(filepath, pdb_format, pdb_root, pdb_version)
			if results:
...
def find_engineered_mutation_file(pdb_file, pdb_root, pdb_version):
	with gzip.open(pdb_file, 'rt') as gz_file:
		for line in gz_file:
			line_lower = line.lower()
			...
			if is_pdb_mutation or is_mmcif_mutation:
				run_cmd(['ln', '-s', pdb_file, pdb_file.replace(os.path.join(pdb_root, pdb_version, 'pdb'), os.path.join(pdb_root, pdb_version, 'mutated_data'))])
				break

def process_match_single_site(items, pdb_format, pdb_root, pdb_version):
	mut, wts = items
	try:
...
def process_match(items, pdb_format, pdb_root, pdb_version, multi_site):
	if multi_site:
		return process_match_multi_site(items)
	return process_match_single_site(items, pdb_format, pdb_root, pdb_version)
...
if __name__ == "__main__":
	...
	if args.re_subset:
		cluster_col = 'cluster_id_30'
		df = pd.read_csv(pair_csv, dtype=str, keep_default_na=False)
		pair_subset_csv = pair_csv.replace('.csv', f'_subset_c{args.n_clusters}.csv')
		selected_cluster_csv = pair_csv.replace('.csv', f'_subset_c{args.n_clusters}_clusters.csv')
		if cluster_col not in df.columns:
			raise ValueError(f"Missing required column: {cluster_col}")
		clusters = sorted(df[cluster_col].drop_duplicates().tolist())
		if len(clusters) < args.n_clusters:
			raise ValueError(
				f"Requested {args.n_clusters} clusters, but only found {len(clusters)} clusters."
			)
		selected_clusters = pd.Series(clusters).sample(n=args.n_clusters, random_state=args.seed).tolist()
		subset_df = df[df[cluster_col].isin(selected_clusters)].copy()
		subset_df.to_csv(pair_subset_csv, index=False)
		pd.DataFrame({cluster_col: selected_clusters}).to_csv(selected_cluster_csv, index=False)
		print(f"Input rows: {len(df)}")
		print(f"Total clusters: {len(clusters)}")
		print(f"Selected clusters: {len(selected_clusters)}")
		print(f"Subset rows: {len(subset_df)}")
		print(f"Subset output: {pair_subset_csv}")
		print(f"Selected cluster list: {selected_cluster_csv}")